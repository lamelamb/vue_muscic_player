...
export default {
  ...
  data () {
    return {
      showFlag: false,
      // 为什么要设置这个，因为scroll 默认的20，会小于中部列表动画计算的时间，导致scroll计算出错
      refreshDelay: 100
    }
  },
  props: {},
  watch: {
    // 切歌成功后滚动至第一个-----本质，什么时候调用this.scrollToCurrent ，之前对一些边界条件过滤一下
    currentSong(newVal, oldVal) {
      if (!newVal.id || !oldVal.id) {
        return
      }
      // 我们这个playList 成功显示了，并且前后2首歌不等
  ...
  methods: {
    ...
    show() {
      ...
      // 延迟计算 better-scroll----better-scroll 必须显示出来
      setTimeout(() => {
        this.$refs.scrollRef.refresh()
      }, 20)

      // 组件刚显示的时候，也要滚动到正在播放的歌曲出
    ...
    // 切歌---- 要 改变当前歌曲currentIndex(是对playList 来说的) ，对palyList 做处理，   playList 和sequence 的关系
