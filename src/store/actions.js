...
export const insertSong = function ({commit, state}, song) {
  ...
  // 归根到底是因为插入了歌曲改变了数据结构，原来的歌曲也发生了变化--- 目标删除原来歌曲并保持当前歌曲不变
  if (fpIndex > -1) {
    if (currentIndex > fpIndex) {
      playlist.splice(fpIndex, 1)
      currentIndex--
    } else {
      playlist.splice(fpIndex + 1, 1) // 此时因为插入了，原来的歌曲的下标就往后倒退一位
    }
  }
...
// 把当前歌曲写进 vuex 最近播放 playHistory 中--- 都是先本地化存储之后，然后再写入vuex 
